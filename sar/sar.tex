...
must define \textit{It} schematically:
\[
\textit{It}[f] = \lam{n}{\lam{x}{\mathsf{R}\ n\ (\lam{q}{\lam{r}{f}})\ x}}
\]
...
in Section~\ref{sec:prevaff}.  Now using \textit{It},
we can define $\delta$ schematically, where $T$ is, as above, a list of length two:
\[
\delta[u] = \textit{It}[\textit{app}_u]\ T\ \textit{apply}
\]
\noindent This is a well-formed definition, because the argument supplied for
the parameter of \textit{It} is closed with respect to parameter $u$.

So we could define \textit{It} and $\delta$.  What breaks down is the
definition of $\Omega$ as $\delta\ \delta$.  Here, as $\delta$ is a
schematic term, it must be supplied with a closed term.  We cannot
write $\delta[\delta]$ because $\delta$ cannot be used as an argument:
it is not a closed term, but requires an argument for its parameter.
So we see that restricting terms to be closed with respect to
...
respect to a list $\Delta$ of definitions.  The rules are the same as in

...
\item The relation $\Delta; \bar{u}\ |\ \bar{x} \vdash
  ...
  arguments). The parameters $\bar{u}$ are considered closed.  The
  variables $\bar{x}$ are locally bound outside $t$ and are also
  considered closed, except that they are dropped in places where
  terms must not contain any free local variables.  This can be
  seen where $\bar{x}$ is replaced with $\cdot$ in premises to the
  rules expressing closedness of $\mathsf{R}$-terms and applications $\mathbf{f}[\bar{t}]$
  of defined symbols.
\end{itemize}

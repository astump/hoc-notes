...
equality proof is being used to change the type of some subterm,
but it ends up blocking reduction or reasoning about the term.
One could have a stuck coercion around a $\lambda$-abstraction,
for example, that is being applied to an argument.  The coercion
would block the $\beta$-reduction step, because it is in between
the function and the argument.  With this approach, however,
the equality proofs are all erased, and such stuck terms do
not arise.  
